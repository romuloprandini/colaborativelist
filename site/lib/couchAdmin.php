...
class couchAdmin {
	...
	*/
	public function createUser ($login, $password, $metadata = array(), $roles = array() ) {
	...
	*/
	public function getUsers($startKey, $endKey) {
		$client = new couchClient( $this->client->dsn() , $this->usersdb, $this->client->options());
		return $client->startkey($startKey)->endkey($endKey)->getView("users","name")->rows;
	}
